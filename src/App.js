...
class App extends React.Component {
  ...
  handleEvaluate() {
    if (!this.state.currentVal.includes("limit")) {
      ...
      let answer = Math.round(100000000000 * eval(expression)) / 100000000000;
      this.setState({
        currentVal: answer.toString(),
        formular:
          expression.replace(/\*/g, "⋅").replace("+", "--") + "=" + answer,
        previousVal: answer,
        evaluated: true,
      });
    }
  ...
  handleOperators(e) {
    if (!this.state.currentVal.includes("limit")) {
      ...
      if (evaluated) {
        ...
        this.setState({
          formular:
            (endsWithNegativeSign.test(formular + value)
              ? formular
              : previousVal) + value,
        });
      } else if (value !== "-") {
  ...
  handleNumbers(e) {
    if (!this.state.currentVal.includes("limit")) {
      ...
      if (currentVal.length > 21) {
        ...
        this.setState({
          currentVal:
            currentVal === "0" || isOperator.test(currentVal) // bug with /-\d\.?/
              ? value
              : currentVal + value,
          formular:
